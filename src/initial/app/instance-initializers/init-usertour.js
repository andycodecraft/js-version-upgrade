...
const setStepAsViewed = (step) => {
    try {
        let shouldStore = !config.branding.cookieConsent;

        if(!shouldStore){
            let cookie_options = JSON.parse(window.Cookies.get('cookie_options'))
            shouldStore = cookie_options && !!cookie_options.strictly_necessary;
        }

        if(shouldStore){
            window.localStorage.setItem('usertour.' + step, (new Date()).toString());
        }
    } catch (e) {
...
export function initialize(application) {
    ...
    tour.on('cancel', function () {
        ...
        if (typeof currentStepId !== 'undefined'){
            setStepAsViewed(currentStepId);

            if(typeof steps[currentStepId].onCancel === 'function') {
    ...
    tour.on('complete', function () {
        ...
        if (typeof currentStepId !== 'undefined'){
            ...
            if(typeof steps[currentStepId].onComplete === 'function') {
