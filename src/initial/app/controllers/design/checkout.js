...
export default Ember.Controller.extend(RegionMixin, PrintShapesMixin, Ember.Evented, EmberValidations, {
    ...
    b2bSite: config.b2b,
    b2bOrderSubmitted: false,
    b2bOrderSuccessful: false,
    ...
    b2bOrderHandler: function () {
        if (this.b2bOrderSubmitted || !this.b2bSite || !this.get('model').b2b_id)
            return;

        this.set('clickedPlaceOrder', true);
        this.trigger('showProgressBar');

        if (!this.get('model').get('shippingOption'))
            return;

        this.placeOrder();
        this.set('b2bOrderSubmitted', true);
    },
    ...
    actions: {
        ...
        createNewPrintRequest: function () {
            var printRequest = this.store.createRecord('printRequest', {
                b2b_id: this.get('model.b2b_id'),
                customer_details: this.get('model.customer_details'),
    ...
    placeOrder () {
        ...
        const openBillingView = () => {
            ...
            if(controller.b2bSite && controller.get('model').b2b_id){
                controller.set('b2bOrderSuccessful', true);
                return;
            }
